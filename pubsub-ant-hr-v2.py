...
def callback(msg):
    print(msg)
    sleep(1)
    # send msg - which in the case of hr is actually just a string
    # representation of an integer
    mqtt_connection.publish(
                topic=args.topic,
                payload='{ "id": 1, "value": ' + str(msg) + '}',
...
def on_message_received(topic, payload, dup, qos, retain, **kwargs):
    ...
    received_count += 1
    # Send message over serial to Arduino USB Serial Cable
    arduinoSerial.write(payload)
...
if __name__ == '__main__':
    ...
    # Continuously check for heart rate values
    # If present then send up through mqtt
    while(1):
        # Read from Ant+ Interface
        # getHeartRateFromAntPlus()
        sleep(1) 
    if args.message:
        if args.count == 0:
